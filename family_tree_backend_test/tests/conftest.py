"""
Shared pytest fixtures for the cleaned family-tree-backend test suite.

Two things to make the FastAPI app importable in tests:

1. core.config now reads env vars at import time and raises if required ones
   are missing. We set them before importing anything from `app`.
2. Neo4j is stubbed (same approach as before) so app startup doesn't try to
   open a live bolt connection.

Auth fixtures are added because every /api/v1/* route now sits behind the
...
# ---------------------------------------------------------------------------
# Neo4j stub — identical contract to the previous test suite. Routes/services
# under test call neo4j_client.run() and we queue the responses up.
# ---------------------------------------------------------------------------
@pytest.fixture
...
def verify_signatures(monkeypatch):
    ...
    fetch so we don't hit the network. Returns a small handle the test can
    use to assert that the fetch was attempted.
    """
    ...
    def fake_fetch_jwks():
        fetched["count"] += 1
        # Shape matches Keycloak's /protocol/openid-connect/certs response,
        # but with no usable keys — every signed-verification path falls
        # through to UNAUTHORIZED_003 when no kid matches.
        return {"keys": []}

    monkeypatch.setattr(auth_bearer, "_fetch_jwks", fake_fetch_jwks)
    # Reset cache so the new fetcher actually runs.
...
def client(neo4j_stub):
    ...
    (e.g. asserting on the unauthenticated response). Most route tests should
    use `authed_client` instead.
    """
...
def authed_client(client, make_token):
    ...
    Use this for the happy path of any /api/v1/* route. Tests that want to
    vary the token per request should use `client` + `make_token` directly.
    """
